...
## Summer 2020: FK added updates to enable 2 year projection boxplot (added proj.years and proj.obj argument). proj.years = 2 expands the x-axis to accommodate a second boxplot. proj.obj is to add a boxplot that was created OUTSIDE of this function.  
## June 2021: JS added long term median on recruit biomass plots and long term median on commercial biomass plots when PA zones are not plotted. 
## Nov 2021: JS added translation to recruit biomass for french plot ~ line 315
###############################################################################################################
# Arguments
...
SSModel.plot.median <- function (x, type = "Model.est", cred.lim = 0.05, ref.pts = NULL, log.R = T, cex = 1,
          Catch.next.year = NULL, pred.lim = NULL, g=NULL,gR = NULL, RP.labels = NULL,BM_scale = "t", proj.years=1, 
          proj.obj = NULL, french=FALSE, ...) 
{
  ...
  if (type == "Model.est") 
  {
    ...
    if (is.null(Catch.next.year)) 
    {
      ...
      if(BM_scale == "t") mtext(text = en2fr("Commercial biomass (meats, tonnes)", translate = french, custom=rosetta_terms_SABHU), side = 2, cex = cex, line = 4)
      lines(x$Years, bounds[1, ], lty = 2)
      lines(x$Years, bounds[2, ], lty = 2)
      #if no reference points plot long term median on time series of commerical biomass 
      if(is.null(ref.pts)) 
      { 
        abline(h=median(x$sims.matrix[,paste0("B[", 1:(length(x$Years)-1), "]")]), col = "blue", lwd=2, lty=2)
      }
      ...
      if (!is.null(ref.pts)) 
      {
        ...
        text(x=RP.labels$x.pos[RP.labels$region == "Healthy"],y=0.9*ylim[2],labels=toupper(en2fr("Healthy", french, custom=rosetta_terms_SABHU)),cex = RP.labels$size[RP.labels$region == "Healthy"],col='chartreuse2')
        text(x=RP.labels$x.pos[RP.labels$region == "Cautious"],y=ref.pts[1]*1.01,labels = toupper(en2fr("Cautious", french, custom=rosetta_terms_SABHU)),cex=RP.labels$size[RP.labels$region == "Cautious"],col='goldenrod1')
        text(x=RP.labels$x.pos[RP.labels$region == "Critical"],y=ref.pts[1]*0.1,labels = toupper(en2fr("Critical", french, custom=rosetta_terms_SABHU)),cex=RP.labels$size[RP.labels$region == "Critical"],col='firebrick1')
      ...
      if (!is.null(ref.pts)) 
      {
        ...
        rect(min(x$Years) - 5, ref.pts[2], max(x$Years) + 
               5, ylim[2] * 1.2, border = NA, col = rgb(0, 
                                                        1, 0, 0.2))
        # Now add the Ref Point label to the figure....
        text(x=RP.labels$x.pos[RP.labels$region == "Healthy"],y=0.99*ylim[2],labels=toupper(en2fr("Healthy", french, custom=rosetta_terms_SABHU)),cex = RP.labels$size[RP.labels$region == "Healthy"],col='chartreuse2')
        text(x=RP.labels$x.pos[RP.labels$region == "Cautious"],y=ref.pts[1]*1.2,labels = toupper(en2fr("Cautious", french, custom=rosetta_terms_SABHU)),cex=RP.labels$size[RP.labels$region == "Cautious"],col='goldenrod1')
        text(x=RP.labels$x.pos[RP.labels$region == "Critical"],y=ref.pts[1]*0.1,labels = toupper(en2fr("Critical", french, custom=rosetta_terms_SABHU)),cex=RP.labels$size[RP.labels$region == "Critical"],col='firebrick1')
      } # end if (!is.null(ref.pts)) 
    ...
    if(!is.null(log.R))
    {
      ...
      if(log.R == T)
      {
        ...
        abline(h=median(x$sims.matrix[,paste0("R[", 1:(length(x$Years)-1), "]")]), col = "blue", lwd=2, lty=2)
      ...
      if(BM_scale == "t") mtext(text = en2fr("Recruit biomass (meats,tonnes)", translate = french, custom=rosetta_terms_SABHU), side = 2, cex = cex, line = 4)
      lines(x$Years, bounds[1, ], lty = 2)
      lines(x$Years, bounds[2, ], lty = 2)
    } # end if(!is.null(log.r))
    mtext(en2fr("Year", french, custom=rosetta_terms_SABHU), side = 1, line = 2, outer = FALSE, cex = cex)
  ...
  if (type == "Survey.est") {
    ...
    lines(x$Years, bounds[1, ], lty = 2)
    lines(x$Years, bounds[2, ], lty = 2)
    mtext(en2fr("Year", french, custom=rosetta_terms_SABHU), side = 1, line = 2, outer = FALSE, cex = 0.8)
    on.exit(par(old.par))
    return()
  }
}