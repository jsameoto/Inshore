...
cruise <- "GM2019"  #note should match year for surveyyear set above 
...
plot(MWTSHGM.YYYY) #update model name
dev.off()
...
condition.ts.plot <- ggplot(GM.con.ts,aes(x=YEAR, y=CONDITION,group_by(STRATA), color=STRATA)) +  
...
#Export plot 
png(paste0(path.directory, assessmentyear, "/Assessment/Figures/SPA6_conditionTrends.png" ))
condition.ts.plot
dev.off()
