...
cruise <- "BF2019"  #note should match year for surveyyear set above 
...
BF.con.ts <- rbind(BF.con.ts %>% select(YEAR, STRATA, CONDITION), mean.depth.bof %>% select("YEAR",STRATA="AREA",CONDITION="Condition"))
...
#..SPA 1A
SPA1A.condition.ts.plot <- ggplot(BF.con.ts[BF.con.ts$STRATA %in% c("MidBaySouth","Zone2to8mile","Zone8to16mile"),], aes(x=YEAR, y=CONDITION,group_by(STRATA), color=STRATA)) +  
  ylim(c(8,16)) +
...
#..SPA 1B
SPA1B.condition.ts.plot <- ggplot(BF.con.ts[BF.con.ts$STRATA %in% c("CapeSpencer","MidBayNorth","UpperBay28C","Combined28D"),], aes(x=YEAR, y=CONDITION,group_by(STRATA), color=STRATA)) +  
  geom_line(aes(linetype=STRATA)) + geom_point(aes(shape=STRATA)) +
  ylim(c(8,16)) +
    xlab("Year") + ylab("Condition (meat weight, g)") + theme_bw() 
...
#.. SPA 4
SPA4.condition.ts.plot <- ggplot(BF.con.ts[BF.con.ts$STRATA=="SPA4",], aes(x=YEAR, y=CONDITION)) +  
  geom_line() + geom_point() + 
  ylim(c(8,16)) + xlim(c(min(BF.con.ts$YEAR)-1,max(BF.con.ts$YEAR)+1)) + 
  xlab("Year") + ylab("Condition (meat weight, g)") + theme_bw() 
