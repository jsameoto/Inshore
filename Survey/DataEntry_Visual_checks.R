...
mwsh <- read.csv("Y:/INSHORE SCALLOP/Survey/2021/data entry templates and examples/GM2021/GM2021_HGTWGT.csv")
str(mwsh)
...
ggplot() + geom_text(data=mwsh
                     [mwsh$Tow>1 & mwsh$Tow<121,]
                     , aes(Height, Weight, colour=as.factor(Tow), label=Num)) 

ggplot() + geom_text(data=mwsh[mwsh$Tow==99 
                               #& mwsh$Tow <50
                               ,], aes(Height, Weight, colour=as.factor(Tow), label=Num))

ggplot() + geom_text(data=mwsh[mwsh$Tow>1& mwsh$Tow<121,], aes(Height, Weight, colour=as.factor(Tow), label=Num)) + facet_wrap(~Tow, scales="free")

ggplot() + geom_text(data=mwsh[mwsh$Tow==75,], aes(Height, Weight, colour=as.factor(Tow), label=Num))



bycatch <- read.csv("Y:/INSHORE SCALLOP/Survey/2021/data entry templates and examples/GM2021/GM2021_bycatch.csv")
...
dhf <-  read.csv("Y:/INSHORE SCALLOP/Survey/2021/data entry templates and examples/GM2021/GM2021_dhf.csv")
...
dhf5 <- dhf[dhf$TOW>99 & dhf$TOW < 121,]
...
check.tows.spatial(cruise="GM2021", year=2021, direct="Y:/INSHORE SCALLOP/Survey/", desktop="NULL", 
                   previouscruisefolder = "data entry templates and examples/GM2019", previouscruisename = "GM2019", plot=TRUE, df=TRUE)
...
test <- subset(double.strata_GM2021, strata==32)
length(unique(test$Oracle.tow..))

test2<- subset(unique(double.strata_GM2021[,1:30]), Strata_id==32)
